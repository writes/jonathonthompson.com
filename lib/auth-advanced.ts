...
export function generateToken(payload: any, expiresIn: string = '1h'): string {
  return sign(payload, process.env.NEXTAUTH_SECRET!, { expiresIn });
}
...
// Removed hardcoded test users for security
// Add users via environment variables or database in production
if (users.length === 0) {
  // No hardcoded users
}
